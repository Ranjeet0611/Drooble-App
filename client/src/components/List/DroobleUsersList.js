...
const DroobleUsersList = (props) => {
  ...
  if (props.droobleUsers.length === 0) {
    return (
      <>
  ...
  return (
    ...
    <InfiniteScroll 
          ...
          useWindow={false}
        >
      {props.droobleUsers.map((item) => (
          <a href={`/User/${item._id}`}>
            <Row p={1.5} gap={2} bgcolor={'#fff'} borderRadius={16}>
              <Item>
                <Avatar classes={avatarStyles} src={item.imageUrl.length>0?item.imageUrl:''} />
              </Item>
              <Info position={'middle'} useStyles={useTutorInfoStyles}>
                <InfoTitle>{item.name}</InfoTitle>
                <InfoSubtitle>{item.country.length>0?item.country:'IN'}</InfoSubtitle>
              </Info>
            </Row>
          </a>
        ))}
...
export default DroobleUsersList;
