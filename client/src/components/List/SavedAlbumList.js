  ...
  if (props.artistAlbum.length === 0) {
    return (
      <>
        ...
        <center>
          <Title level={5}>No Saved Album</Title>
        </center>
      </>
    );
  }
              ...
              <Item>
                <a href={item.album.external_urls.spotify}>
                  <div className={avatarStyles.root}>
                    <Avatar src={item.album.images.length>0?item.album.images[0].url:''} />
                  </div>
                </a>
              </Item>
              <Info useStyles={useMusicInfoStyles} minWidth={0}>
                <InfoTitle>{item.album.name.split('(')[0]}</InfoTitle>
                <InfoSubtitle>{item.album.release_date}</InfoSubtitle>
              </Info>
