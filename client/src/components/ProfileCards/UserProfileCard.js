    ...
    <Card className={cx(styles.card, shadowStyles.root)}>
      <CardContent>
        <Avatar
          ...
          src={props.userProfileData.imageUrl.length>0?props.userProfileData.imageUrl:''}
        />
        <h3 className={styles.heading}>{props.userProfileData.name}</h3>
        <span className={styles.subheader}>
          {props.userProfileData.country.length>0?props.userProfileData.country:'IN'}
        </span>
      </CardContent>
      <Divider light />
      <Box display={'flex'}>
        <Box p={2} flex={'auto'} className={borderedGridStyles.item}>
          ...
          <p className={styles.statValue}>{props.userProfileData.country.length>0?props.userProfileData.country:'IN'}</p>
        </Box>
        <Box p={2} flex={'auto'} className={borderedGridStyles.item}>
          <p className={styles.statLabel}>Followers</p>
          <p className={styles.statValue}>{props.userProfileData.followers}</p>
        </Box>
      </Box>
    </Card>
  );
